  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // Check for session-based API key (Gemini Live/VEO requirement)
    ...
    <div className="min-h-screen bg-[#06080f] text-white flex flex-col selection:bg-indigo-500/30">
        ...
        {currentView === 'generate' && (
          <div className="space-y-12 animate-in fade-in duration-500">
            <div className="bg-slate-900/40 border border-slate-800 rounded-[3.5rem] p-12 shadow-2xl relative overflow-hidden">
              ...
              {extendingVideo && (
                <div className="mb-8 p-6 bg-indigo-500/10 border border-indigo-500/20 rounded-3xl flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className="w-16 h-16 rounded-xl overflow-hidden bg-black border border-indigo-500/30">
                      <video src={extendingVideo.url} className="w-full h-full object-cover" />
                    </div>
                    <div>
                      <p className="text-xs font-black uppercase tracking-widest text-indigo-400 mb-1">Extending Clip</p>
                      <p className="text-sm font-medium text-slate-300 line-clamp-1 italic">"{extendingVideo.prompt}"</p>
                    </div>
                  </div>
                  <button 
              ...
              <textarea 
                ...
                placeholder={extendingVideo ? "Describe what happens next..." : "Describe the cinematic scene you want to create..."}
              ...
              >
                ...
                <span>{isGenerating ? status || 'Processing...' : (extendingVideo ? 'Extend Timeline' : 'Create Masterpiece')}</span>
              </button>
            </div>
          ...
          <div className="max-w-4xl mx-auto py-12 text-center animate-in slide-in-from-bottom duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <div className="bg-slate-900 border border-slate-800 p-12 rounded-[4rem] space-y-8">
                ...
                <button className="w-full py-5 bg-slate-800 rounded-3xl font-black uppercase tracking-widest text-xs">Active Plan</button>
              </div>
              <div className="bg-indigo-600 p-12 rounded-[4rem] space-y-8 shadow-2xl shadow-indigo-600/30">
                ...
                <button className="w-full py-5 bg-white text-indigo-600 rounded-3xl font-black uppercase tracking-widest text-xs hover:scale-105 transition-transform">Get Access</button>
              </div>
            </div>
          </div>
        )}
      ...
      <footer className="py-8 px-12 border-t border-slate-900 flex flex-col md:flex-row items-center justify-between gap-6 opacity-40 hover:opacity-100 transition-opacity">
        ...
        <div className="flex gap-10">
          <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" className="text-[10px] font-black uppercase tracking-widest hover:text-indigo-400">Billing Policy</a>
          <span className="text-[10px] font-black uppercase tracking-widest">Lumina v1.2.5</span>
        </div>
      </footer>
    </div>
  );
