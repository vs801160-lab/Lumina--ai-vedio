...
class SupabaseService {
  ...
  async recordTransaction(transaction: Transaction) {
    ...
    if (this.isConfigured) await this.client!.from('transactions').insert([transaction]);
  }
  ...
  async saveVideo(video: GeneratedVideo, userId: string) {
    ...
    if (this.isConfigured && userId !== 'demo-user') {
      await this.client!.from('videos').insert([{ ...video, user_id: userId, is_public: false }]);
    }
    return video;
  }
  ...
  async fetchPublicVideos() {
    ...
    // Mock public videos for demo
    return JSON.parse(this.safeGetItem('lumina_library') || '[]').filter((v: any) => v.isPublic);
  }
