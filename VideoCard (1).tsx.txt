  ...
  return (
    <div className="bg-slate-900/60 rounded-[2.5rem] overflow-hidden border border-slate-800 hover:border-indigo-500/50 hover:shadow-[0_0_40px_rgba(99,102,241,0.1)] transition-all group flex flex-col">
      <div className="aspect-video bg-black relative overflow-hidden">
        <video ref={videoRef} src={video.url} className="w-full h-full object-cover" onEnded={() => setIsPlaying(false)} poster={video.url + "#t=0.5"} />
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 backdrop-blur-[1px]">
          <button onClick={togglePlay} className="w-16 h-16 bg-white/10 hover:bg-white/20 border border-white/20 rounded-full flex items-center justify-center transition-all scale-90 hover:scale-100 backdrop-blur-md">
      ...
      <div className="p-7 flex-grow flex flex-col">
        ...
        <p className="text-sm text-slate-300 line-clamp-2 mb-8 font-medium leading-relaxed italic opacity-80">"{video.prompt}"</p>
