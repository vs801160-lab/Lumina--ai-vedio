...
import { Play, Video, LayoutDashboard, User, LogOut, Plus, Zap } from 'lucide-react';
    ...
    <nav className="sticky top-0 z-[70] bg-[#080c16]/80 backdrop-blur-xl border-b border-slate-800/50 px-8 py-5 flex items-center justify-between">
      ...
      <div className="flex items-center gap-8">
        ...
        <div className="flex items-center gap-4">
          <div className="bg-slate-900 border border-slate-800 pl-4 pr-1 py-1 rounded-2xl flex items-center gap-3 shadow-inner">
            <div className="flex flex-col">
              <span className="text-[8px] font-black text-slate-500 tracking-widest uppercase">{t.credits}</span>
              <span className="text-sm font-black text-indigo-400 leading-none">{credits}</span>
            </div>
            <button 
              onClick={onBuyCredits}
              className="bg-indigo-600 hover:bg-indigo-500 p-2 rounded-xl text-white transition-all group/btn"
              title="Add Credits"
            >
