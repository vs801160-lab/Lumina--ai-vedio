...
const App: React.FC = () => {
  ...
  const startGeneration = async () => {
    ...
    try {
      ...
      if (err.message === "KEY_RESET_REQUIRED" || err.message.toLowerCase().includes("leaked")) {
        setHasApiKey(false);
        setIsKeyLeaked(true);
        setError("Your API key was reported as leaked or the project is invalid. Please select a fresh key from a paid project.");
      } else if (err.message.includes("BILLING_ERROR")) {
        setError("BILLING REQUIRED: Your Google Cloud project must have a paid billing account linked to download videos. Go to console.cloud.google.com/billing to setup.");
      } else {
          ...
          <div className="max-w-md space-y-10 animate-in fade-in zoom-in duration-500">
            ...
            <div className="space-y-4">
              ...
              <p className="text-slate-400 font-medium leading-relaxed">
                {isKeyLeaked 
                  ? "Google has disabled this key due to public exposure. Use a NEW key from a project with active billing." 
              ...
              {error && (
                <div className="p-8 bg-red-500/10 border border-red-500/20 rounded-[2.5rem] mb-8 flex flex-col gap-4 animate-in fade-in duration-300">
                  ...
                  {error.includes("BILLING") && (
                    <div className="mt-4 p-6 bg-slate-950/50 rounded-3xl border border-white/5 space-y-4">
                      <div className="flex items-center gap-2 text-indigo-400">
                        <Info size={18} />
                        <span className="text-xs font-black uppercase tracking-widest">How to fix this:</span>
                      </div>
                      <ol className="text-[11px] text-slate-400 uppercase tracking-widest leading-loose space-y-2">
                        <li>1. Open <a href="https://console.cloud.google.com/billing" target="_blank" className="text-indigo-400 underline inline-flex items-center gap-1">Cloud Billing <ExternalLink size={10}/></a></li>
                        <li>2. Select the project associated with your API key.</li>
                        <li>3. Ensure an active credit card or billing account is linked.</li>
                        <li>4. Veo models require "Paid Tier" status to download files.</li>
                      </ol>
                    </div>
                  )}
                </div>
