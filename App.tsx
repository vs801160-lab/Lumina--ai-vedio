...
import { 
  AlertCircle, Loader2, Languages, Zap, Wand2, Globe, CheckCircle2, ShieldCheck, Plus, Info, Sparkles
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const refreshData = async (uid: string) => {
    const userCredits = await db.getCredits(uid);
  ...
  const handlePayment = (pkg: CreditPackage) => {
    ...
    const razorpayKey = process.env.RAZORPAY_KEY_ID || 'rzp_test_dummy';
    
    if (razorpayKey === 'rzp_test_dummy') {
      console.warn("Using Razorpay Test Key. Replace with your real Key ID in .env for production.");
    }
    ...
    const options = {
      ...
      handler: async (response: any) => {
        try {
          // Record successful payment and update credits in database
          await db.updateCredits(
            user.id, 
            pkg.credits, 
            `Purchased ${pkg.name} - Ref: ${response.razorpay_payment_id}`
          );
          setSuccessMsg(t.success_pay);
          refreshData(user.id);
          setTimeout(() => setSuccessMsg(null), 5000);
        } catch (e) {
          setError("Failed to sync credits. Please contact support.");
        }
      },
      prefill: {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await db.updateCredits(user.id, -10, `Generated video: ${newVideo.id}`);
      refreshData(user.id);
      setCurrentView('library');
      setPrompt('');
    } catch (e: any) {
      setError(e.message);
      if (e.message?.includes("API")) setHasApiKey(false);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#06080f] text-white flex flex-col font-sans selection:bg-indigo-500/30 overflow-x-hidden">
      <Navbar currentView={currentView} onNavigate={setCurrentView} credits={credits} tier={tier} user={user} t={t} onLoginClick={() => db.signInWithGoogle().then(r => setUser(r.user))} onLogout={() => { db.signOut(); setUser(null); setCredits(0); }} onBuyCredits={() => setCurrentView('pricing')} />
      ...
      {hasApiKey === false && (
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-6 backdrop-blur-3xl">
          <div className="max-w-md w-full bg-slate-900 border border-slate-800 p-10 rounded-[3rem] text-center space-y-6 shadow-2xl">
            ...
            <h2 className="text-3xl font-black uppercase tracking-tighter">Verification Pending</h2>
            <p className="text-slate-400 text-sm">Google Cloud verification mein time lagta hai. Jab tak key active nahi hoti, aap feature test nahi kar payenge.</p>
        ...
        {currentView === 'generate' && (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="text-center space-y-4">
              ...
              <p className="text-slate-400 uppercase tracking-[0.3em] font-bold text-[10px]">{t.sub}</p>
            </div>
            ...
            <div className="bg-slate-900/40 border border-slate-800 rounded-[3rem] p-6 md:p-12 shadow-2xl backdrop-blur-3xl relative">
              ...
              {error && <div className="p-6 bg-red-500/10 border border-red-500/20 rounded-3xl mb-8 flex items-center gap-4 text-red-400 font-medium text-sm"><AlertCircle size={20} />{error}</div>}
        ...
        {currentView === 'library' && (
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {library.map(v => <VideoCard key={v.id} video={v} onDelete={(id) => { setLibrary(library.filter(x => x.id !== id)); db.deleteVideo(id); }} onExtend={() => {}} onPinReference={() => {}} onTogglePublic={async (id, val) => { await db.updateVideoPrivacy(id, val); refreshData(user.id); }} />)}
              ...
              {PACKAGES.map(pkg => (
                <div key={pkg.id} className={`p-1 rounded-[3.5rem] ${pkg.recommended ? 'bg-indigo-500' : 'bg-slate-800'}`}>
                  <div className="bg-[#0b0e1a] rounded-[3.4rem] p-10 flex flex-col h-full text-center">
                    ...
                    <button onClick={() => handlePayment(pkg)} className="w-full py-5 bg-indigo-600 rounded-2xl font-black uppercase tracking-widest shadow-lg active:scale-95 transition-transform">Buy Now</button>
                  </div>
                </div>
              ))}
            </div>
