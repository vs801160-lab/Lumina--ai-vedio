...
const App: React.FC = () => {
  ...
  const startGeneration = async () => {
    ...
    try {
      ...
      console.error("App layer error:", errMsg);
      ...
      } else if (errMsg.includes("BILLING_ERROR")) {
        setError("BILLING REQUIRED: Your Google Cloud project must have a paid billing account linked to download videos.");
      } else {
        setError(errMsg);
      }
    } finally {
                  ...
                  {error.includes("BILLING") && (
                    <div className="mt-4 p-6 bg-slate-950/50 rounded-3xl border border-white/5 space-y-4">
                      ...
                      <ol className="text-[11px] text-slate-400 uppercase tracking-widest leading-loose space-y-2">
                        <li>1. Open <a href="https://console.cloud.google.com/billing" target="_blank" rel="noreferrer" className="text-indigo-400 underline inline-flex items-center gap-1">Cloud Billing <ExternalLink size={10}/></a></li>
                        <li>2. Link a Paid project to your API Key.</li>
                        <li>3. Free accounts cannot download raw Veo video files.</li>
                      </ol>
                    </div>
                  )}
                </div>
