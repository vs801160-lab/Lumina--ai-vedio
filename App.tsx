...
const App: React.FC = () => {
  ...
  // Temporal & Visual Reference states for advanced generations
  ...
  useEffect(() => {
    // Initial Auth and Key selection checks
  ...
  const startGeneration = async () => {
    ...
    try {
      ...
      // Clear visual references after successful generation
      ...
      // Implement reset and prompt for key on Requested entity not found error
                 ...
                 <div className="flex items-center justify-between mb-6">
                    ...
                    {previousVideo && (
                      <div className="flex items-center gap-2 bg-indigo-500/20 px-3 py-1 rounded-full border border-indigo-500/30">
                        ...
                        <button onClick={() => setPreviousVideo(null)} className="hover:text-white text-indigo-400 transition-colors"><X size={12} /></button>
                      </div>
                    )}
                 </div>
                 <textarea 
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {library.map(video => (
                    <VideoCard 
                      ...
                      onPinReference={(url) => {
                        // Use video as a visual start reference
                        setStartImage(url);
                        setCurrentView('generate');
                        window.scrollTo(0, 0);
                      }} 
