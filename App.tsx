...
const ROADMAP_STEPS = [
  { id: 1, title: 'Deploy to Cloud', icon: Rocket, desc: 'Connect GitHub and make your site live on the web.', status: 'DONE' },
  ...
  const startGeneration = async () => {
    ...
    try {
      ...
      if (err.message === 'API_KEY_ERROR') {
        setError("API key expired. Please re-select.");
        handleKeySelection();
      } else {
    ...
    <div className="min-h-screen bg-[#080c16] text-white relative font-sans flex flex-col">
            ...
            <div className="lg:col-span-8 space-y-6">
              <div className="bg-slate-900/60 border border-slate-800 rounded-[2.5rem] p-8">
                ...
                <textarea 
                  ...
                  className="w-full h-40 bg-slate-950 border border-slate-800 rounded-2xl p-6 text-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-slate-500">{t.negativePrompt}</label>
                    <input type="text" value={negativePrompt} onChange={(e) => setNegativePrompt(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-sm" />
                  </div>
                  <div className="space-y-2">
              ...
              <div className="bg-slate-900/40 border border-slate-800 rounded-[2rem] p-8">
                <h4 className="text-xs font-black uppercase text-slate-500 mb-6">Style</h4>
                <div className="grid grid-cols-3 md:grid-cols-6 gap-4">
                  {STYLES.map(s => (
                    <button key={s.id} onClick={() => setSelectedStyle(s)} className={`p-4 rounded-2xl border transition-all flex flex-col items-center gap-2 ${selectedStyle.id === s.id ? 'bg-indigo-600 border-indigo-400' : 'bg-slate-950 border-slate-800'}`}>
            ...
            <div className="lg:col-span-4 space-y-6">
                ...
                <div className="space-y-6">
                  <div className="flex gap-2">
                    {['16:9', '9:16', '1:1'].map(ratio => (
                      <button key={ratio} onClick={() => setAspectRatio(ratio as AspectRatio)} className={`flex-grow py-3 rounded-xl border text-xs font-bold ${aspectRatio === ratio ? 'bg-indigo-600 border-indigo-400' : 'bg-slate-950 border-slate-800'}`}>
                        {ratio}
                      </button>
                    ))}
                  </div>
                  <button onClick={startGeneration} disabled={isGenerating} className="w-full py-6 bg-gradient-to-br from-indigo-600 to-purple-700 rounded-3xl font-black text-xl shadow-2xl flex items-center justify-center gap-3 disabled:opacity-50">
              ...
              <div className="bg-slate-900/40 border border-slate-800 rounded-[2rem] p-6 text-center">
                ...
                <p className="text-xs font-medium text-slate-400 leading-relaxed italic">
                  "Video generation takes up to 2 minutes."
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="space-y-6">
              {ROADMAP_STEPS.map((step) => (
                <div key={step.id} className="bg-slate-900/60 border border-slate-800 p-8 rounded-[2.5rem] flex items-center gap-8">
      ...
      {showPricing && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-md bg-black/60">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-5xl rounded-[3rem] p-10 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {PRICING_PLANS.map(plan => (
                <div key={plan.id} className={`p-8 rounded-[2.5rem] border relative flex flex-col ${plan.popular ? 'bg-indigo-600/10 border-indigo-500' : 'bg-slate-950 border-slate-800'}`}>
                  {plan.popular && <span className="absolute -top-4 left-1/2 -translate-x-1/2 bg-indigo-500 text-white text-[10px] font-black px-4 py-1 rounded-full">POPULAR</span>}
                  ...
                  <button onClick={() => handleBuyCredits(plan.credits)} className="w-full py-4 rounded-2xl font-black bg-indigo-600 hover:bg-indigo-500 text-white">
                    BUY NOW
                  </button>
                </div>
              ))}
      ...
      {showLogin && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-md bg-black/60">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-[3rem] p-10 relative">
            ...
            <div className="text-center mb-10">
              ...
              <h3 className="text-3xl font-black mb-2 uppercase tracking-tighter">{t.loginTitle}</h3>
              <p className="text-slate-400 text-sm">{t.loginDesc}</p>
            </div>
            <button onClick={handleLogin} className="w-full py-5 bg-white text-black rounded-2xl font-black text-sm flex items-center justify-center gap-3">
      ...
      <footer className="border-t border-slate-800 bg-[#080c16] py-12 px-6 mt-20">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex items-center gap-3 font-black tracking-tighter uppercase">LUMINA AI</div>
          <div className="flex items-center gap-8 text-xs font-bold text-slate-500">
            <button onClick={() => setCurrentView('roadmap')} className="hover:text-indigo-400">Roadmap</button>
          </div>
          <div className="text-[10px] text-slate-600 font-medium uppercase tracking-widest">© 2024 Lumina. {t.footerRights}</div>
        </div>
      </footer>
    </div>
  );
