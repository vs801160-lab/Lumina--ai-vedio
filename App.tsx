...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (u) {
        setUser(u);
        const c = await db.getCredits(u.id);
        setCredits(c);
        setTier(await db.getTier(u.id));
        setLibrary(await db.fetchVideos(u.id));
      }
    };
    init();
  ...
  const handlePayment = (pkg: CreditPackage) => {
    ...
    if (!razorpayKey || razorpayKey === 'undefined' || !razorpayKey.startsWith('rzp_')) {
      setError("Razorpay Key ID missing or invalid! Ensure it starts with 'rzp_'. Rebuild the app after adding it to .env");
      return;
    }
