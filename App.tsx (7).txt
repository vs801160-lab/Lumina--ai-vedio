...
import { 
  AlertCircle, Loader2, Languages, Zap, Wand2, Globe, CheckCircle2, ShieldCheck, Plus, Info, Sparkles, CreditCard
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handlePayment = (pkg: CreditPackage) => {
    ...
    // Key Validation
    let razorpayKey = process.env.RAZORPAY_KEY_ID?.trim();
    
    if (!razorpayKey || razorpayKey === 'undefined' || !razorpayKey.startsWith('rzp_')) {
      setError("Galat Razorpay Key! Dashboard se 'Key ID' copy karein (rzp_test... se shuru hoti hai).");
      return;
    }
    ...
    try {
      const options = {
        ...
        description: `${pkg.credits} Credits Top-up`,
        image: "/favicon.svg",
        handler: async (response: any) => {
          ...
          try {
            await db.updateCredits(
              user.id, 
              pkg.credits, 
              `Purchased ${pkg.name} - ID: ${response.razorpay_payment_id}`
            );
            setSuccessMsg(t.success_pay);
            refreshData(user.id);
            setTimeout(() => setSuccessMsg(null), 5000);
          } catch (e) {
            setError("Credits update nahi ho paye. Support ko contact karein.");
          }
        },
        prefill: {
          name: user?.user_metadata?.full_name || "",
          email: user?.email || "",
          contact: "" // User can fill this
        },
        theme: {
          color: "#6366f1",
        }
      ...
      rzp.on('payment.failed', function (response: any) {
        setError(`Payment Fail ho gaya: ${response.error.description}`);
        console.error("Payment Fail Error:", response.error);
      });

      rzp.open();
    } catch (err: any) {
      setError("Razorpay start nahi ho saka. Refresh karke try karein.");
  ...
  return (
    <div className="min-h-screen bg-[#06080f] text-white flex flex-col font-sans selection:bg-indigo-500/30 overflow-x-hidden">
      ...
      {/* Error Alert */}
      {error && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[100] bg-red-500 text-white px-8 py-4 rounded-3xl shadow-2xl flex items-center gap-3 animate-in fade-in slide-in-from-top-4">
          <AlertCircle size={20} />
      ...
      {hasApiKey === false && (
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-6 backdrop-blur-3xl">
          <div className="max-w-md w-full bg-slate-900 border border-slate-800 p-10 rounded-[3rem] text-center space-y-6 shadow-2xl">
            ...
            <h2 className="text-3xl font-black uppercase tracking-tighter">API Key Missing</h2>
            <p className="text-slate-400 text-sm">Video generation ke liye Gemini API Key zaroori hai. Please Select Project Key.</p>
