...
export class GeminiVideoService {
  ...
  static async generateVideo(
    ...
    try {
      ...
      // Using template string for key concatenation as per Gemini Docs
      ...
      if (!response.ok) {
        ...
        console.error("Video Download Failed:", { status, errorBody });
        ...
        throw new Error(`Download failed with status ${status}. Ensure project billing is active.`);
      }
      ...
      console.error("GeminiVideoService Error:", msg);

      if (msg === "BILLING_REQUIRED") {
        throw new Error("BILLING_ERROR: Your Google Cloud project needs an active billing account (Paid Tier) to download generated videos.");
      }
